...
STL分解 (Seasonal-Trend decomposition using Loess) および重回帰分析を用いた検証を行った. 
...
この結果は, 特定の期間（第一波）におけるショックが極めて強力であることを示唆している. 
しかしながら, 本回帰モデルのDurbin-Watson比は $1.377$ であり, 残差に正の自己相関が残存していることが示唆される. 
これは, 単純な回帰分析では時系列データの動的な構造を十分に制御できていない可能性を示しており, 
次節以降で導入する状態空間モデルによる厳密なモデリングの必要性を裏付けるものである. 
